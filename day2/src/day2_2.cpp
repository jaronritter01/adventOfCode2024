...
Vector2D *generatePermutations(std::vector<int> *inputVec);
...
/**
 * Function to clean up a 2D vector.
 * 
...
/**
 * Generate a 2D vector of permutation vectors.
 * 
 * @param inputVec A pointer to the vector you want to generate permutations for.
 * @return 2D vector of permutations of the input vec by removing a single value.
 */
Vector2D *generatePermutations(std::vector<int> *inputVec) {
    Vector2D *output = new Vector2D();
    for (int i = 0; i < inputVec -> size(); i++) {
        std::vector<int> *copy = new std::vector<int>(*inputVec);
...
int calcNumSafeReports(const Vector2D &reports) {
    ...
    for (std::vector<int> *report : reports) {
        if (!isReportSafe(report)) {
            // Check to see if any permutation passes
            bool anyPermutationPass = false;
            Vector2D *permutations = generatePermutations(report);
            for (std::vector<int> *permutation : *permutations) {
                if (isReportSafe(permutation)) {
                    anyPermutationPass = true;
                    break;
                }
            }

            if (!anyPermutationPass) {
                numSafeReports -= 1;
            }
            deleteVector2D(permutations);
        }
    }
